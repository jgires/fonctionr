...
-   Ajouter `n_weight_upp` et `n_weight_low` + harmoniser les noms des colonnes entre fonctions (`n` vs `n_tot`, `n_weighted` vs `n_tot_weighted`...) : je propose l'ordre suivant: les variables de ventilation, l'indicateur, l'indicateur_low, l'indicateur_up, n_sample, n_true_weighted, n_true_weighted_low, n_true_weighted_upp, n_tot_weighted, n_tot_weighted_low, n_tot_weighted_upp
...
-   Bypasser l'erreur du test stat avec `tryCatch()`.
-   ajouter la possibilité d'un na.rm.prop = FALSE pour que la proportion soit calculée sur l'ensemble des individus (les NA étant comptabilisés dans le dénominateur)
