...
-   <https://notstatschat.rbind.io/2021/07/22/subsets-and-subpopulations-in-survey-inference/>
    ...
    -   De ce fait, j'ai inclu une option de filtre (`filter_exp`) dans les fonctions (qui filtre après la déclaration du design), qui évite de filtrer l'objet avant en dégradant le design. =\> Vérifier que c'est bien OK !
...
-   Ajouter des checks pour les inputs :
    ...
    1.  [***X C'EST FAIT***]{.underline} Existence des colonnes dans le dataframe ;
...
-   Où mettre le % dans les différentes fonctions (dans la barre mais avec un if pour quand la barre est trop petite)

-   Bien mettre le n = (en dessous de la barre)

    -   tentative mais marche pas à tous les coups

-   [***X C'EST FAIT***]{.underline} Ajouter les trucs que François avait modifié dans les fonctions de Joël

## central_group

-   Bypasser l'erreur du test stat avec `tryCatch()`
    -   déjà fait pour `crosstable()`, à faire pour les autres
...
-   Bypasser l'erreur du test stat avec `tryCatch()`
    -   déjà fait pour `crosstable()`, à faire pour les autres
-   [***X C'EST FAIT***]{.underline} Une expression qui contient `is.na(x)` ne fonctionne pas, car je supprime les `NA` sur les variables de l'expression ! =\> Exclure la possibilité d'entrer la fonction `is.na()` dans l'expression
...
-   Ajouter un filtre
    ...
    -   C'est fait, mais il faut ajouter des conditions d'arret si certain caractères ne fontionnent pas <!--# J'ai ajouté un stop si les variables de filtre n'existent pas dans data, c'est ce que tu voulais dire ? -->

-   Quid des NA ? couleur distincte? Si oui à faire <!--# Je l'ai fait, comme pour les autres fonctions : NA en gris clair -->

-   Quid des NA dans les facets? Les supprimer? faire une catégorie à part? Actuellement, il en fait une catégorie à part. <!--# J'ai ajouté : si on met na.rm = T alors ça supprime aussi sur les facet, car laisser le NA sur facet et pas sur quali_var est étrange -->

-   Concernant le réordonne ment des modalités, le choix a été fait de ne pas laisser la possibilité de réordonner. <!--# J'ai ajouté l'option. Il fallait bouger le NA en bas, ce qui demandait la même chose que pour réordonner les catégories => autant le faire aussi -->
