...
Pour la fonction `esth_graph()`, il n'y a pas d'argument `data`, mais un argument `tab` qui doit correspondre à la table avec les indicateurs (il s'appelle `tab` car il est similaire à l'objet `tab` produit par la plupart des fonctions de `fonctionr`).<!--# J'enlèverais cette partie -->

Les exemples ci-dessous montrent trois manière différentes de prendre en compte un même plan de sondage : en l'indiquant directement dans la fonction de `fonctionr`, en créant d'abord un objet `survey` et en créant d'abord un objet `srvyr`.<!--# Simplifier l'affichage avec patchwork -->
...
Pour `esth_graph()`, les arguments `var` et `value` sont obligatoires et ils indiquent respectivement les colonnes dans `tab` avec nom de l'indicateur et sa valeur.<!--# J'enlèverais cette partie -->
...
-   `moustache_probs` est un argument qui permet de déterminer les quantiles pour calculer et indiquer la "moustache" sur le graphique dans `distrib_group_continuous()`. La moustache est une représentations graphique qui reprend différentes proportions de la populations autour de la médiane. [Le vecteur doit reprendre d'abord les proportions les plus importantes et puis les proportions les plus limitées]{.underline} <!--# Ah ça je ne sais pas, je ne crois pas ? Sinon il faut faire en sorte que l'ordre ne soit pas important ! A VERIFIER ! -->. Les valeurs par défaut sont 0.95, 0.80 et 0.50 de manière à représenter pour chaque groupe 95%, 80% et 50% de la population autour de la médiane.
...
Dans l'exemple ci-dessous nous comparons les revenus du travail des travailleuses et travailleurs à temps partiel en modifiant de nombreuses options graphiques. `bw` étant plus faible, la distribution est moins lissée et on peut y observer des variations de densités plus locales (mais pas toujours statistiquement significatives). `resolution` étant plus faible, on peut déceler les droites qui composent les deux courbes de fréquence. `limits` permet de se concentrer sur la plage de valeur (de -500€ à 3500€) qui contient la grande majorité des observations. Dans le graphique, on a indiqué la médiane sous la forme d'un point et d'une ligne. Ses intervalles de confiance sont aussi indiqués par une barre d'erreur (au niveau de l'axe des X), deux lignes en pointillées et une aire colorées. Les déciles sont indiqués par des lignes verticales grises et on a masqué la moustache. Notez que que le revenu du travail mensuel est directement calculé dans l'expression en additionnant le salaire avec le revenu d'une activité indépendante avant de diviser le tout par 12.
