...
Ci-dessous, la fonction `distrib_d()` décrit la distribution des différents statuts socio-économiques dans le dataframe `eusilc`, avec une le poids `rb050`[^1]. Comme on le voit, les variables sont indiquées sans mentionner le dataframe (`var` au lieu de `df$var`) et sans guillemets, à la manière de la syntaxe du `tidyverse`.
...
```{r}
eusilc_prop_sex <- prop_group(eusilc,
  group = pl030_rec,
  prop_exp = py090n > 0,
  group.fill = rb090,
  weights = rb050,
...
tableau_recap <- tibble::tribble(
  ~`Catégorie d’argument`, ~Site,         ~type,      ~prop_group,  ~many_val_group,       ~many_val, ~central_group,          ~distrib_group_c,            ~distrib_c, ~distrib_group_d,     ~distrib_d,
     ...
     "Esthétique de base",    1L,     "logical",       "show_lab",       "show_lab",      "show_lab",     "show_lab",                "show_lab",            "show_lab",       "show_lab",     "show_lab",
     "Esthétique de base",    1L,      "string",     "total_name",               NA,              NA,   "total_name",                        NA,                    NA,               NA,             NA,
  ...
  "Esthétique: graphique",    1L,      "string",               NA,               NA,              NA,         "fill",                        NA,                    NA,               NA,         "fill",
  "Esthétique: graphique",    1L,      "string",            "pal",            "pal",           "pal",             NA,                     "pal",                 "pal",            "pal",             NA,
  "Esthétique: graphique",    1L,     "numeric",      "direction",      "direction",     "direction",             NA,                        NA,                    NA,      "direction",             NA,
  ...
  "Esthétique: graphique",    1L,     "numeric",          "dodge",          "dodge",         "dodge",        "dodge",                        NA,                    NA,          "dodge",        "dodge",
    ...
    "Esthétique: lettres",    1L,     "numeric", "wrap_width_leg", "wrap_width_leg",              NA,             NA,       "to_delete UTILE ?",                    NA, "wrap_width_leg",             NA,
    "Esthétique: lettres",    1L,     "numeric",    "legend_ncol",    "legend_ncol",              NA,             NA,       "to_delete UTILE ?",                    NA,    "legend_ncol",             NA,
     ...
     "Esthétique: labels",    1L,      "string",     "legend_lab",     "legend_lab",              NA,             NA, "to_delete A IMPLEMENTER",                    NA,     "legend_lab",             NA,
...
-   Les couleurs des barres peuvent être indiquées dans les arguments `fill` et `pal`.
    ...
    -   `fill` est utilisé pour les graphiques monochrome (`central_group()` et ses alias, `distrib_discrete()`<!--# et esth_graph(), mais déplacé dans une autre page -->). Les couleurs de base de R ou un code couleur hexadécimal peuvent être indiqués dans `fill`.
    -   `pal` est utilisé pour les graphiques qui peuvent présenter plusieurs couleurs. `pal` doit alors généralement prendre le nom d'une palette des packages `MetBrewer`, `MoMAColors` ou `PrettyCols` (sauf pour la distribution d'une variable quantitative, voir ci-dessous). L'argument `direction` permet d'inverser le sens de la palette, en remplaçant le `1`, par défaut, par `-1` pour avoir le sens opposé.
...
Dans certains cas, on peut indiquer dans `pal` soit une couleur unique, soit une palette des packages `MetBrewer`, `MoMAColors` ou `PrettyCols`. C'est le cas pour `many_val()` et ses alias, où `pal` peut aussi prendre une couleur unie (couleur de R ou code hexadécimal) si l'utilisateur souhaite un graphique monochrome. C'est également le cas pour `prop_group()` : lorsque l'on indique des sous-groupes dans l'argument `group.fill`, il faut pouvoir différencier les sous-groupes avec une couleur, et alors il faut indiquer une palette des trois packages mentionnés. Par contre, lorsqu'il n'y a pas de sous-groupes, il faut indiquer une couleur unique. L'exemple ci-dessous montre les deux usages de `pal` avec `prop_group()`. On calcule la proportion de travailleurs à temps partiel selon le sexe. Dans le premier graphique, on différencie les proportions selon la région ; `pal` prend alors le nom d'une palette (ici `"Neon"` de `PrettyCols`). Dans le deuxième, il n'y a pas de sous-groupes. `pal` n'a alors besoin que d'une unique couleur, et on a indiqué la couleur de base de R `"gold"`.

```{r}
# Avec sous-groupes
eusilc_prop_sub_group <- prop_group(eusilc,
  group = rb090,
  prop_exp = pl030_rec == "Working part time",
  group.fill = db040,
  weights = rb050,
  title = "% of ind. working part time by sex & region",
  subtitle = "(with sub-groups)",
  pal = "Neon"
)
# Sans sous-groupes
eusilc_prop_no_sub_group <- prop_group(eusilc,
  group = rb090,
  prop_exp = pl030_rec == "Working part time",
  weights = rb050,
  title = "% of ind. working part time by sex",
