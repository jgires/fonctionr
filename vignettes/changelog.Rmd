...
-   Documenter tous les arguments des différentes fonctions + vérifier que les explications sont bien correctes (quelques erreurs repérées, par ex. à cause de copier-coller). **Il manque les descriptions générales des fonctions `many_prop()` et `many_val_group()`**.

-   Vérifier que les scripts sont bien commentés pour qu'on se rappelle ce qu'on a fait (il manque des notes).

-   Passer le code de chaque fonction en revue à l'occasion de la vérification des commentaires =\> **de gros bugs encore découverts ! (copier-coller de fonctions avec de mauvais noms d'arguments !)**

-   L'import des packages extérieurs n'est pas OK : `MetBrewer` et `MoMAColors` masquent des fonctions de base et sont par ailleurs en conflit (message d'erreur au chargement de `fonctionr`) = Par ailleurs, il est inutile d'importer tous les packages extérieurs via le NAMESPACE =\> Voir <https://r-pkgs.org/dependencies-in-practice.html>.
