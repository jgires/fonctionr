...
-   Documenter tous les arguments des différentes fonctions + vérifier que les explications sont bien correctes (quelques erreurs repérées, par ex. à cause de copier-coller, et il manque les descriptions générales des fonctions `many_prop()` et `many_val_group()`). Il y a également des champs manquants (voir warnings de `roxygenise()`). Voir ici pour savoir les différentes options possibles dans la doc : <https://r-pkgs.org/man.html>.\
    **=\> POUR FRANCOIS : caractères illégaux ajoutés dans les explications des commentaires, je crois que ce sont les guillemets français (`")` =\> faire tourner un check pour voir !**

-   Vérifier que les scripts sont bien commentés pour qu'on se rappelle ce qu'on a fait (il manque des notes).

-   Passer le code de chaque fonction en revue à l'occasion de la vérification des commentaires =\> de gros bugs encore découverts ! (copier-coller de fonctions avec de mauvais noms d'arguments, erreurs qui font bugger le `na.rm.group`, etc.)
...
-   Pour les checks : mieux préciser les variables incriminées ? Ce n'est pas le cas pour les checks des variables nécessaires, par ex. Elles ne sont pas nombreuses, mais c'est plus sympa pour l'utilisateur.
