...
-   **(Joël) Le fonctionnement de `theme_fonctionr()` avec `theme == NULL` par défaut est pas terrible =\> changer.**
...
-   **(Joël) Le changement du code pour les palettes fait qu'on peut désormais introduire une palette lorsqu'il y a une seule couleur (ex. : `central_group()`)**
...
-   **(Joël) Bug à l'import de `fonctionr` du fait de collision entre `PrettyCols` et les fonctions de base ? Je ne comprends pas le message.**
