...
-   **(Joël) L'utilisation de `ggtext` a été implémentée de manière un peu bricolée (comme un code en "surcouche"). Ça fonctionne, mais le code est largement simplifiable (mieux pour la lisibilité). Si on l'implémente de manière durable, à revoir. Par exemple, voir solution proposée ici : <https://github.com/wilkelab/ggtext/issues/121>.\
    =\> Code simplifié pour `prop_group()`, faire pareil pour les autres fonctions !**

-   **(Joël) Faire une fonction commune pour la création de palettes + checks =\> redondance inutile entre les fonctions.**

-   **(Joël) Laisser la possibilité d'introduire des palettes personnalisées dans les fonctions.**
...
-   **(Joël) Les cluster / strates / weights n'apparaissent pas dans le message de la console avec les replicates =\> pourquoi ? (prioritaire vu usage par Statbel)**
...
-   **(Joël) Bug à l'import de `fonctionr` du fait de collision entre `MetBrewer`, `MoMAColors`, `PrettyCols` et les fonctions de base ? Je ne comprends pas le message.\
    =\> CRAN oblige à ne pas utiliser des packages pas sur CRAN =\> retirer MoMAColors ?**
...
-   **(François) Ajouter des exemples pour chaque fonction (pour le site) : manque `pivot_longer_survey()`, `esth_graph()`.**
