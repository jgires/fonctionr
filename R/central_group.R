...
#' @param reorder `TRUE` if you want to reorder the groups according to the mean/median. `NA` value, if `na.rm.group = FALSE`, is not included in the reorder. In case of facets, the groups are reordered based on each median group. Default is `FALSE`.
...
#' @param caption Caption of the graphic. This caption goes under de default caption showing the result of the statistical test. There is no way of not showing the result of the chi-square test as a caption.
...
central_group <- function(data,
  ...
  # On recalcule quanti_exp dans une variable unique si c'est une expression a la base (apres le select ci-dessus pour ne pas l'effacer)
  # => necessaire pour les tests stat ci-dessous
  ...
  graph <- fonctionr_ggplot_labs(
    graph = graph,
    xlab = xlab,
    lang_note_axis_x = if(type == "mean") lang_mean else lang_median,
