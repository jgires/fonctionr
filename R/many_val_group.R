...
#' @param ylab Y label on the graphic. As [ggplot2::coord_flip()] is used in the graphic, `ylab` refers to the y label on the graphic, after the [ggplot2::coord_flip()], and not to the y variable in the data. Default (`ylab = NULL`) displays the name of the groups variable (if `position = 'dodge'` or `'stack'`) or no Y axe label (if `position = 'flip'`). To show no Y label, use `ylab = ""`.
