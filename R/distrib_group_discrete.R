...
#' @param reorder TRUE if you want to reorder the groups according to the proportion of the first level of quali_var.  NA group, in case if na.rm.group = FALSE, is not included in the reorder. In case of facets, the groups are reordered based on each median group. Default is FALSE.
#' @param show_n TRUE if you want to show on the graphic the number of observations in the sample in each category (of quali_var) of each group. FALSE if you don't want to show this number. Default is FALSE.
...
#' @param show_labs TRUE if you want to show axes and legend labels. FALSE if you don't want to show any labels on axes and legend. Default is TRUE.
...
#' @param legend_lab Legend (fill) label on the graphic. Default (legend_lab = NULL) displays the name of the discrete variable (quali_var). To show no legend label, use legend_lab = "".
...
#' @param export_path Path to export the results in an xlsx file. The file includes three (without facets) or two sheets (with facets): the table, the graphic and the Chi-Square statistical test result.
#'
...
distrib_group_discrete <- function(data,
  ...
  graph <- fonctionr_ggplot_labs(
    graph = graph,
    xlab = xlab,
    lang_note_axis_x = lang_distribution,
    x_exp = deparse(substitute(quali_var)),
    ylab = ylab,
    legend_lab = legend_lab,
    wrap_width_leg = wrap_width_leg,
    show_labs = show_labs
