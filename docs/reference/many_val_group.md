...
ggplot graphic. The confidence intervals are taking into account the
...
- type:
  ...
  `"prop"` to compute proportions by group ; `"mean"` to compute means
  by group ; `"median"` to compute medians by group.
...
- na.vars:
  ...
  The treatment of `NA` values in variables (`list_vars`). `"rm"`
  removes `NA` seperately in each individual variable, `"rm.all"`
  removes every individual that has at least one `NA` in one variable.
  Default is `"rm"`.

- total:

  `TRUE` if you want to compute a total, `FALSE` if you don't. Default
  is `TRUE`. Total is not displayed nor computed if `position = 'flip'`.
...
- position:
  ...
  Position adjustment for the ggplot. Default is `"dodge"`. Other
  possible values are `"flip"` and `"stack"`. `"dodge"` means that
  groups are on the y axe and variables are in differents colors,
  `"flip"` means that variables are on the y axe and groups are in
  differents colors, and `"stack"` means that groups are on the y axe
  and variables are stacking with differents colors. The latter is
  usefull when the variables are component of a broader sum variable
  (e.g. different sources of income). If `position = 'flip'`, total is
  not displayed nor computed. If `position = "stack"`, confidence
  intervals are never shown on the graphic.

- show_ci:

  `TRUE` if you want to show the error bars on the graphic. `FALSE` if
  you do not want to show the error bars. Default is `TRUE`. If
...
- show_n:
  ...
  in the sample for each group and variable. The number can varie
  between variables if `na.vars = "rm"`. `FALSE` if you do not want to
...
- scale:
  ...
  as percentages. This argument is only used in case of `type = "prop"`.
...
- dodge:
  ...
  `position = 'dodge'` or variables in case of `position = 'flip'`).
  There is always no space between sub-groups.
...
- wrap_width_y:
  ...
  axe (groups if `position = 'dodge'` or `'stack'`, variables if
  `position = 'flip'`). Default is `25`.

- wrap_width_leg:

  Number of characters before going to the line for the labels the
  legend (variables if `position = 'dodge'` or `'stack'`, groups if
  `position = 'flip'`). Default is `25`.
...
- xlab:
  ...
  `lang = "en"`) or "Aandeel:" (if `lang = "nl"`) folowed by the names
  of the variables (`list_vars`). To show no X label, use `xlab = ""`.
...
- ylab:
  ...
  displays the name of the groups variable (if `position = 'dodge'` or
  `'stack'`) or no Y axe label (if `position = 'flip'`). To show no Y
  label, use `ylab = ""`.
...
- legend_lab:
  ...
  name of the groups variable (if `position = 'flip'`). To show no
