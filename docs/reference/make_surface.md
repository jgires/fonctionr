...
Function that represents the values of a group variable as areas
...
- error_low:
  ...
  interval. If either `error_low` or `error_upp` is `NULL` error
  rectangles are not shown on the graphic.
...
- facet:
  ...
  A variable in `tab` defining the faceting group, if applicable.
  Default is `NULL`.
...
- position:
  ...
  The position of the rectangles: `"mid"` for center alignment,
...
- col:
  ...
  Color of the rectangles if the user wants a monocolor graph. `col`
  must be a R color or an hexadecimal color code. As `pal` has a
  priority over `col`, if the user wants to use `col`, he must not use
  simultaneously the `pal` argument (even `pal = NULL`).

- pal:

  Colors of the rectangles if the user wants the rectangles to have
  different colors. `pal` must be vector of R colors or hexadecimal
  colors or a palette from packages MetBrewer or PrettyCols or a palette
  from fonctionr. `pal` has a priority over `col`.
...
- bg:
  ...
  Color of the background. `bg` must be a R color or an hexadecimal
  color code.
...
- ratio:
  ...
  squares ; greater than `1` produces vertical rectangles and smaller
  than `1` produces horizontal rectangles. Default is `3/2`.
...
- hjust.title:
  ...
  value. Default (`0`) leads to left alignment, `1` leads to right
  alignment and `0.5` leads to centered alignement.
