...
(`dens`), a central value table (`tab`), a quantile table (`quant`) and
a ready-to-be published ggplot graphic (`graph`).

The density table contains x-y coordinates to draw a density curve. The
central value table contains the median or the mean of the continuous
variable, with its confidence interval, the sample size and the
estimation of the total, with its confidence interval. The quantile
table contains quantiles and their confidence intervals. The quantiles
and the limits are used as thicks on the X axe of the graphic. The
...
- quanti_exp:
  ...
  described. Notice that any observations with `NA` in at least one of
  the variable in `quanti_exp` are excluded for the computation of the
  density and of the indicators.
...
- limits:
  ...
  of indicators (median/mean and quantiles). Default is `NULL` to show
  the entire distribution on the graphic.
...
- ylab:
  ...
  Y label on the graphic. If `ylab = NULL`, Y label on the graphic will
  be `"Densité"` (if `lang = "fr"`), `"Density"` (if `lang = "en"`) or
